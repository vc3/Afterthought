...
namespace Afterthought.Amender
{
    ...
    internal class AssemblyAmender : MutatingVisitor
    {
        ...
        Dictionary<string, List<ITypeAmendment>> _typeAmendments;
            ...
            : base(host, true)
        {
            ...
            _typeAmendments = typeAmendments.GroupBy(w => w.Type.FullName).ToDictionary(gr => gr.Key, gr => gr.ToList());
        ...
        bool AmendType(NamedTypeDefinition type)
        {
            ...
            List<ITypeAmendment> typeAmendments;
            _typeAmendments.TryGetValue(type.ToString(), out typeAmendments);

            // Exit immediately if the type is not being amended
            if (typeAmendments == null)
            {
            ...
            foreach (ITypeAmendment typeAmendment in typeAmendments)
            {
                // Attributes
                ...
                foreach (var interfaceType in typeAmendment.Interfaces)
                {
                    ...
                    foreach (var property in interfaceType.GetProperties())
                    {
                        ...
                        var existingProperty = type.Properties == null ? null : type.Properties.FirstOrDefault(p => AreEquivalent(p, property) && p.Visibility == TypeMemberVisibility.Public);
                    ...
                    foreach (var method in interfaceType.GetMethods())
                    {
                        ...
                        var existingMethod = type.Methods == null ? null : type.Methods.FirstOrDefault(m => AreEquivalent(m, method) /*&& m.Visibility == TypeMemberVisibility.Public*/);
        ...
        {
            // If we want to compare to a property that come from an Interface we have to trim the namespace.
            //int trimIndex = propertyDef.Name.Value.LastIndexOf(".", StringComparison.Ordinal) != -1 ? propertyDef.Name.Value.LastIndexOf(".", StringComparison.Ordinal) + 1 : 0;
            return

                // Ensure property names match
                //propertyDef.Name.Value == property.Name &&
                propertyDef.Name.Value/*.Substring(trimIndex)*/ == property.Name &&
